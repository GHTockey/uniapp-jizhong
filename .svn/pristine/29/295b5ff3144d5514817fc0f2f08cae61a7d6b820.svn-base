	...
	<view class="present_card_container">
		...
		<view class="present_card_list">
			<view class="present_card_item" v-for="(item, index) in currentItemList" :key="index">
				<!-- 图片 -->
				<image class="w-full rounded-md"
					src="https://saas.jizhongkeji.com/static/jzkj/static/images/present_item.png" mode="widthFix">
				</image>
				<!-- 文字 -->
				...
				<view class="flex justify-between mt-[25rpx]">
					...
					<view class="flex gap-[20rpx]">
						<template v-if="item.is_used == 0">
							<view class="tce_theme_btn text-[24rpx]" @click="toPage('/pages/mine/presentSend')">赠送朋友
							</view>
							<view class="tce_theme_empty_btn text-[24rpx]"
								@click="toPage('/pages/mine/presentActivate')">立即使用</view>
						</template>
